...
import { SystemType, DeviceType } from "../enums/auth.enums";
...
export class AuthService {
	...
	private async assertValidSystemDevice(
	...
	async signOut(payload: any) {	
		...
		if (token.isRevoked) {
			throw new BadRequestException({
				error: 402,
				message: "This accesstoken has been revoked",
			});
		}
	...
	async refresh(data: any) {
		...
		try {
			...
			if (token.isRevoked) {
				...
				throw new BadRequestException({
					error: 402,
					message: "This refreshtoken has been revoked",
				});
			}
	...
	async signIn(data: any) {
		...
		try {
			...
			this.logger.log({
				...
				userId: user.id,
				identifierType: data.username.includes("@")
					? "email"
					: /^\+?[0-9\s-]+$/.test(data.username)
						? "phone"
						: "username",
				deviceType: data.deviceType,
				systemType: data.systemType,
